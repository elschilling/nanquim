...
| `public/assets/img/nanquim-command-icons.svg` | Original command-tool mask sheet created in the repository in August 2026. It provides one distinct 24-pixel technical-drafting glyph for every registered command. | Project-authored from new paths with no external artwork. Distributed under Nanquim's GPLv3 license and tinted at runtime through `currentColor`. |
